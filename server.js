...
const server = http.createServer(async (req, res) => {
  ...
  if (pathname === '/api/flutterwave/webhook' && req.method === 'POST') {
    try {
      ...
      console.log('[Flutterwave Webhook Received]:', payload.event, payload.data ? payload.data.id : '');

      res.writeHead(200, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ status: 'success', message: 'Webhook received' }));
    } catch (err) {
      res.writeHead(500, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ status: 'error', message: err.message }));
    }
